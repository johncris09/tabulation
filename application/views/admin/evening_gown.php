    ...
    $(document).ready(function(){  
      ...
      setInterval(function(){  
        ...
        load_rank();
          ...
          $.ajax({
            type : 'POST',
            url : BASE_URL + "evening_gown/is_judge_done_scoring",
            data : {  
              judge : judge 
            },
            dataType: "json",
      ...
      function consolidate_rank(){
        $('.candidate-consolidate').each(function(){   
          ...
          $.ajax({
            type : 'POST',
            url : BASE_URL + "evening_gown/get_candidate_consolidated_score",
            data : { 
      ...
      function get_tot_score(){
        $('.tot-score').each(function(){   
          ...
          $.ajax({
            type : 'POST',
            url : BASE_URL + "evening_gown/get_tot_score",
            data : { 
              candidate : $(this).data('candidate'), 
            },
            dataType: "json",
      ...
      function load_rank(){ 
        $.ajax({
          url: BASE_URL + 'evening_gown/get_consolidated_ranking',
          type: 'POST',   
          dataType: 'JSON',
          success: function(data){   
      ...
      $('.print-score-summary').on('click', function(){
        ...
        $.ajax({
          ...
          success : function(data){  
            if(data){
              window.open(BASE_URL + "evening_gown/result_judge_score/" + judge, "_blank")
            }else{ 
              Swal.fire({
      ...
      $('#print-summary').on('click', function(){ 
        $.ajax({
          type : 'POST',
          url : BASE_URL + "evening_gown/is_all_done_scoring", 
          dataType: "json",
          success : function(data){ 
            if(data){
              window.open(BASE_URL + "evening_gown/print_summary", "_blank")
              // window.open( BASE_URL + "evening_gown/print_summary" , "Print Summary", "toolbar=yes,scrollbars=yes,resizable=yes,top=150,left=450,width=870,height=630");
            }else{ 
              Swal.fire({
                title: "Unavailable this time",
                icon: 'error',
      ...
      $('#print-result').on('click', function(){
        $.ajax({
          ...
          success : function(data){
            if(data){
              window.open(BASE_URL + "evening_gown/result", "_blank")
              // window.open( BASE_URL + "evening_gown/result" , "Print Result", "toolbar=yes,scrollbars=yes,resizable=yes,top=100,left=600,width=600,height=870");
            }else{ 
              Swal.fire({
                title: "Unavailable this time",
                icon: 'error',
          ...
          if (result.value) {  
            $.ajax({
              ...
              success: function(data){    
                if(data){ 
                $.ajax({
                  url: BASE_URL + 'evening_gown/unlock',
                  type: 'POST',   
                  data: _this.data(),
                  dataType: 'JSON',
                  success: function(data){    
