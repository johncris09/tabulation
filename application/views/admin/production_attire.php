                      ...
                      <?php
                        foreach($judge as $row){
                          echo '
                            <th>
															'.$row['short_name'].'
                              <i style="font-size: 15px;" class="unlock bx bx-lock-open-alt text-danger font-weight-bolder lock-toggle" title="Unlock" data-table="talent_presentation" title="Unlock"  data-judge="'.$row['id'].'"></i>
    ...
    $(document).ready(function(){  
      ...
      setInterval(function(){  
        ...
        load_rank();
          ...
          $.ajax({
            type : 'POST',
            url : BASE_URL + "production_attire/is_judge_done_scoring",
            data : {  
              judge : judge 
            },
            dataType: "json",
      ...
      function consolidate_rank(){
        $('.candidate-consolidate').each(function(){   
          ...
          $.ajax({
            type : 'POST',
            url : BASE_URL + "production_attire/get_candidate_consolidated_score",
            data : { 
      ...
      function get_tot_score(){
        $('.tot-score').each(function(){   
          ...
          $.ajax({
            type : 'POST',
            url : BASE_URL + "production_attire/get_tot_score",
            data : { 
              candidate : $(this).data('candidate'), 
            },
            dataType: "json",
      ...
      function load_rank(){ 
        $.ajax({
          url: BASE_URL + 'production_attire/get_consolidated_ranking',
          type: 'POST',   
          dataType: 'JSON',
          success: function(data){   
      ...
      $('.print-score-summary').on('click', function(){
        ...
        $.ajax({
          ...
          success : function(data){  
            if(data){
              window.open(BASE_URL + "production_attire/result_judge_score/" + judge, "_blank")
            }else{ 
              Swal.fire({
          ...
          success : function(data){ 
						if(data){
              window.open(BASE_URL + "production_attire/result", "_blank")
							// window.open( BASE_URL + "production_attire/result" , "Print Result", "toolbar=yes,scrollbars=yes,resizable=yes,top=100,left=600,width=600,height=870");
						}else{ 
							Swal.fire({
								title: "Unavailable this time",
								icon: 'error',
						...
						if (result.value) {  
							$.ajax({
								...
								success: function(data){    
									if(data){ 
									$.ajax({
										url: BASE_URL + 'production_attire/unlock',
										type: 'POST',   
										data: _this.data(),
										dataType: 'JSON',
										success: function(data){    
