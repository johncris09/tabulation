    ...
    $(document).ready(function(){  
      ...
      function consolidate_rank(){
        $('.candidate-consolidate').each(function(){   
          ...
          $.ajax({
            type : 'POST',
            url : BASE_URL + "swim_wear/get_candidate_consolidated_score",
            data : { 
      ...
      function get_tot_score(){
        $('.tot-score').each(function(){   
          ...
          $.ajax({
            type : 'POST',
            url : BASE_URL + "swim_wear/get_tot_score",
            data : { 
              candidate : $(this).data('candidate'), 
            },
            dataType: "json",
      ...
      function load_rank(){ 
        $.ajax({
          url: BASE_URL + 'swim_wear/get_consolidated_ranking',
          type: 'POST',   
          dataType: 'JSON',
          success: function(data){   
            ...
            console.info(xhr.responseText);
				...
				$('#print-summary').on('click', function(){ 
					$.ajax({
						...
						success : function(data){ 
							if(data){
                window.open(BASE_URL + "swim_wear/print_summary", "_blank")
								// window.open( BASE_URL + "swim_wear/print_summary" , "Print Summary", "toolbar=yes,scrollbars=yes,resizable=yes,top=150,left=450,width=870,height=630");
							}else{ 
								Swal.fire({
									title: "Unavailable this time",
									icon: 'error',
			  ...
			  $('#print-result').on('click', function(){
					$.ajax({
						...
						success : function(data){
            if(data){
              window.open(BASE_URL + "swim_wear/result", "_blank")
								// window.open( BASE_URL + "swim_wear/result" , "Print Result", "toolbar=yes,scrollbars=yes,resizable=yes,top=100,left=600,width=600,height=870");
							}else{ 
								Swal.fire({
									title: "Unavailable this time",
									icon: 'error',
