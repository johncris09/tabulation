                    ...
                    <tr> 
                      ...
                      <?php
                        foreach($judge as $row){
                          echo '
                            <th>
															'.$row['short_name'].'  <i style="font-size: 15px;" class="bx bx-lock text-danger font-weight-bolder unlock" title="Unlock" data-table="top_five" data-judge-id="'.$row['id'].'"></i>
														</th>
                          ';
                        } 
                      ?>
    ...
    $(document).ready(function(){  
      ...
      function consolidate_rank(){
        $('.candidate-consolidate').each(function(){   
          ...
          $.ajax({
            type : 'POST',
            url : BASE_URL + "top_five/get_candidate_consolidated_score",
            data : { 
      ...
      function get_tot_score(){
        $('.tot-score').each(function(){   
          ...
          $.ajax({
            type : 'POST',
            url : BASE_URL + "top_five/get_tot_score",
            data : { 
              candidate : $(this).data('candidate'), 
            },
            dataType: "json",
      ...
      function load_rank(){ 
        $.ajax({
          url: BASE_URL + 'top_five/get_consolidated_ranking',
          type: 'POST',   
          dataType: 'JSON',
          success: function(data){   
