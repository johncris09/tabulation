...
class Evening_gown extends CI_Controller {
    ...
    public function insert()
    {
		...
		// else insert
		$record = $this->evening_gown_model->if_exist($data);
		...
		if( $record->num_rows() > 0){ 
			...
			$update = $this->evening_gown_model->update($update_score); 
			...
			$insert = $this->evening_gown_model->insert($data);
			if($insert){ 
				$data = array(
					'response' => true,
	...
	function get_candidate_score()
	{
		...
		$get_candidate_score = $this->evening_gown_model->get_candidate_score($data);
	...
	function get_candidate_consolidated_score()
	{
		...
		$get_candidate_score = $this->evening_gown_model->get_candidate_consolidated_score($data);
	...
	function get_tot_score()
	{
		...
		$get_tot_score = $this->evening_gown_model->get_tot_score($data);
	...
	function get_candidate_score_consolidate($candidate_id)
	{
		...
		$arrcore = $this->evening_gown_model->get_candidate_score_consolidate($data);
		return $arrcore;
	...
	function get_ranking_specific_judge()
	{
		...
		$result = $this->evening_gown_model->get_ranking_specific_judge($data);
	...
	function get_consolidated_ranking()
	{
		...
		$result = $this->evening_gown_model->get_consolidated_ranking($data);
		$x = $result;
	...
	function update_rank()
	{
		...
		$update = $this->evening_gown_model->update_rank($data); 
	...
	{
		// get top 3 candidate
		$candidate = $this->evening_gown_model->get_top_candidate();
	...
	public function delete_previous_score()
	{
		...
		if($record->num_rows() > 0){
			$this->evening_gown_model->delete_previous_score($data);
		}
	...
	{ 
		...
		$data['candidate'] = $this->candidate_model->get_all_candidate();
		$data['judge'] = $this->user_model->get_all_judge();  
		$this->load->view('admin/evening_gown_summary', $data);
	...
	function get_candidate_rank_summary($candidate, $judge)
	{
		...
		$rank = $this->evening_gown_model->get_candidate_rank_summary($data); 
	...
	function get_summary_candidate_final_rank($candidate, $judge = 0)
	{
		...
		$final_rank= 0; 
		$get_final_rank = $this->evening_gown_model->get_summary_candidate_final_rank($data);
